...
BCTRANSFER_ACCOUNT = os.getenv("INERY_ACCOUNT")
TOKEN_SYMBOL = os.getenv("TOKEN_SYMBOL")
BCTRANSFER_QUANTITY = os.getenv("BCTRANSFER_QUANTITY")
BCTRANSFER_MEMO = os.getenv("_BCTRANSFER_MEMO")
...
async def bctransfer_token(token_contract, bctransfer, quantity, memo):
    try:
        ...
        print(f"ERROR: Can't bctransfer token symbol {TOKEN_SYMBOL}")
        print(f"DETAILS: {error.details[0]['message']}")

bctransfer_token( TOKEN_CONTRACT , BCTRANSFER_ACCOUNT , BCTRANSFER_QUANTITY , BCTRANSFER_MEMO )