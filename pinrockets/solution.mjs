...
const url = "http://35.187.233.36:8888"

const json_rpc = new JsonRpc(url) 
const private_key = process.env.PRIVATE_KEY 
const actor = process.env.INERY_ACCOUNT

const account = "pinrockets"
...
async function executeCRUD(id, user, data){
    await createRecord(id, user, data)
    await destroyRecord(id)
}

executeCRUD(1, account, "CRUD Transaction example via JSON RPC")
