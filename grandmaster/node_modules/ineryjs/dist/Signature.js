...
var Signature = /** @class */ (function () {
    ...
    /** Instantiate Signature from an INERYIO-format Signature */
    Signature.fromString = function (sig, ec) {
        var signature = ineryjs_numeric_1.stringToSignature(sig);
        if (!ec) {
            ec = ineryjs_key_conversions_1.constructElliptic(signature.type);
        }
    ...
    Signature.fromElliptic = function (ellipticSig, keyType, ec) {
        ...
        var ineryioRecoveryParam;
        if (keyType === ineryjs_numeric_1.KeyType.k1 || keyType === ineryjs_numeric_1.KeyType.r1) {
            ineryioRecoveryParam = ellipticSig.recoveryParam + 27;
            if (ellipticSig.recoveryParam <= 3) {
                ineryioRecoveryParam += 4;
            }
        }
        else if (keyType === ineryjs_numeric_1.KeyType.wa) {
            ineryioRecoveryParam = ellipticSig.recoveryParam;
        }
        var sigData = new Uint8Array([ineryioRecoveryParam].concat(r, s));
        if (!ec) {
    ...
    Signature.prototype.toElliptic = function () {
        ...
        if (this.signature.type === ineryjs_numeric_1.KeyType.k1 || this.signature.type === ineryjs_numeric_1.KeyType.r1) {
            ellipticRecoveryBitField = this.signature.data[0] - 27;
            if (ellipticRecoveryBitField > 3) {
                ellipticRecoveryBitField -= 4;
            }
        }
        else if (this.signature.type === ineryjs_numeric_1.KeyType.wa) {
    ...
    /** Export Signature as INERYIO-format Signature */
    Signature.prototype.toString = function () {
        return ineryjs_numeric_1.signatureToString(this.signature);
    };
    ...
    Signature.prototype.recover = function (data, shouldHash, encoding) {
        ...
        return ineryjs_key_conversions_1.PublicKey.fromElliptic(ellipticKPub, this.getType(), this.ec);
    };
    return Signature;
}());
