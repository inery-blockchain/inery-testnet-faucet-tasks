...
class Publicprimarychema(StringSchema):
    pass
...
class processSchema(colander.MappingSchema):
    ...
    process_extensions = ExtensionsSchema()

# signed process


class SignedprocessSchema(colander.MappingSchema):
...
class PushprocessSchema(colander.MappingSchema):
    process_id = StringSchema()
    broadcast = BooleanSchema()
    process = SignedprocessSchema()


class processsSchema(colander.SequenceSchema):
    processs = processSchema()
...
class Abirequirementchema(colander.MappingSchema):
    new_type_name = StringSchema()
    type = StringSchema()


class AbirequirementSchema(colander.SequenceSchema):
    default = []
    missing = []
    requirement = Abirequirementchema()
...
class AbiSchema(colander.MappingSchema):
    ...
    requirement = AbirequirementSchema()
...
class AbiType(BaseObject):
    def __init__(self, d):
        self._validator = Abirequirementchema()
...
class AbiTable(BaseObject):
    ...
    def encode(self):
        ...
        key_requirement = self._encode_buffer(self.key_requirement)
        type = self._encode_buffer(self.type)
        return '{}{}{}{}{}'.format(name, index_type, key_names, key_requirement, type)
...
class Abi(BaseObject):
    ...
    def __init__(self, d):
        ...
        self.requirement = self._create_obj_array(self.requirement, AbiType)
    ...
    def get_raw(self):
        ...
        # encode requirement
        requirement = self._encode_buffer(self.requirement)
        ...
        return '{}{}{}{}{}{}{}{}{}'.format(version, requirement, structs, actions, tables,
...
class process(BaseObject):
    def __init__(self, d, chain_info, lib_info):
        ...
        # validate
        self._validator = processSchema()
        super(process, self).__init__(d)
        # parse actions
    ...
    def encode(self):
        ...
        actions = self._encode_buffer(self.actions)
        trans_exts = self._encode_buffer(self.process_extensions)
...
class Packedprocess:
    ...
    def decode_actions(self, buf):
        ...
        while cnt < length and length:
            ...
            # get abi information
            contract_abi = self._user.get_abi(acct_name)
    ...
    def decode_trx_extensions(self, buf):
        ...
        (length, ctx_buf) = self._decode_buffer(VarUInt(), buf)
        if length > 0:
            raise NotImplementedError("Currently inerypy does not support process extensions")
        # get length of action array
    ...
    def get_process(self):
        ...
        if not self._is_unpacked:
            ...
            self._unpacked_trx['process_extensions'] = trx_ext
            # set boolean
...
class AbiType(BaseObject):
    def __init__(self, d):
        self._validator = Abirequirementchema()
        super(AbirequirementSchema, self).__init__(d)
