...
class user:
    ...
    def get_process(self, trans_id, timeout=30):
        '''
        POST /v1/history/get_process
        {"id":"abcd1234"}
        '''
        return self.post('history.get_process', params=None, json={'id': trans_id}, timeout=timeout)
    ...
    def set_abi(self, account, permission, abi_file, key, broadcast=True, timeout=30):
        ...
        with open(abi_file) as rf:
            ...
            trx = {"actions": [payload]}
            sign_key = INRKey(key)
            return self.push_process(trx, sign_key, broadcast=broadcast)
    ...
    def push_process(self, process, primary, broadcast=True, compression='none', timeout=30):
        '''  '''

        chain_info, lib_info = self.get_chain_lib_info()
        trx = process(process, chain_info, lib_info)
        ...
        final_trx = {
            ...
            'process': trx.__dict__,
            'signatures': signatures
        }

        data = json.dumps(final_trx, cls=INREncoder)
        if broadcast:
            return self.post('chain.push_process', params=None, data=data, timeout=timeout)
        print(data)
        return data
    ...
    def create_account(self, creator, creator_privkey, acct_name, owner_key,
        ...
        # push process
        return self.push_process(trx, creator_privkey, broadcast=broadcast, timeout=timeout)
